...
class TestStateManager:
    ...
    def test_was_failure(self, h, state_manager, stage, status):
        h["status"] = status
        h["stage"] = stage
        was_changed, finished_stage = state_manager.transition(h, was_failure=True)
        assert was_changed == True, '"was_changed" should be True'
        assert finished_stage == False, '"finished_stage" should be False'
        assert (
    ...
    def test_running_up_down(self, h, state_manager, i_stage, up, f_stage, f_status):
        h["stage"] = i_stage
        h["status"] = STATUS.RUNNING
        was_changed, finished_stage = state_manager.transition(h, up=up)
        assert was_changed == True, '"was_changed" should be True'
        assert finished_stage == True, '"finished_stage" should be True'
