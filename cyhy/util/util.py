__all__ = ['ranges', 'list_to_range_string', 'range_string_to_list', 'pp', 'to_json', 'copy_attrs', 
          'Enumerator', 'isXML', 'count_file_lines', 'clean_mongo_key',
           'safe_divide', 'safe_percent', 'pretty_bail', 'time_to_utc', 'time_to_local', 'setup_logging',
           'report_dates', 'utcnow', 'warn_and_confirm']
...
def report_dates(now=None):
    ...
    if now.month >= 10: # FY starts in October
        fy_start = now + relativedelta(month=10, day=1, hour=0, minute=0, second=0, microsecond=0)
    else:
        fy_start = now + relativedelta(years=-1, month=10, day=1, hour=0, minute=0, second=0, microsecond=0)
    ...
    week_start = now + relativedelta(weekday=SU(-1), hour=0, minute=0, second=0, microsecond=0)
    week_end = week_start + relativedelta(day=7, microseconds=-1)
    prev_week_start = now + relativedelta(weekday=SU(-2), hour=0, minute=0, second=0, microsecond=0)
    prev_week_end = week_start - relativedelta(microseconds=1)
    
    return {'now':now, 
            'fy_start':fy_start, 'fy_end':fy_end, 
            'prev_fy_start':prev_fy_start, 'prev_fy_end':prev_fy_end,
