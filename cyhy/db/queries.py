        ...
        [
            ...
            },
            {
                "$group": {
                    ...
                    "medium": {
                        "$sum": {"$cond": [{"$eq": ["$_id.severity", 2]}, 1, 0]}
                    },
                    "high": {"$sum": {"$cond": [{"$eq": ["$_id.severity", 3]}, 1, 0]}},
                    "critical": {
                        "$sum": {"$cond": [{"$eq": ["$_id.severity", 4]}, 1, 0]}
                    },
                    "total": {"$sum": 1},
                }
            },
        ...
        [
            {
                "$match": {
                    ...
                    "service.name": {"$exists": True, "$ne": "unknown"},
                }
            },
            {
                "$group": {
        ...
        [
            {"$match": {"latest": True, "_id": {"$nin": snapshots_to_exclude}}},
            {
                "$group": {
                    ...
                    "unique_vulnerabilities_low": {
                        "$sum": "$unique_vulnerabilities.low"
                    },
                    "unique_vulnerabilities_medium": {
                        "$sum": "$unique_vulnerabilities.medium"
                    },
                    "unique_vulnerabilities_high": {
                        "$sum": "$unique_vulnerabilities.high"
                    },
                    "unique_vulnerabilities_critical": {
                        "$sum": "$unique_vulnerabilities.critical"
                    },
                    "unique_vulnerabilities_total": {
                        "$sum": "$unique_vulnerabilities.total"
                    },
                }
            },
            {
