...
class VulnTicketManager(object):
    ...
    def __check_false_positive_expiration(self, ticket, time):
        ...
        if ticket['false_positive'] == True:
            ...
            if fp_expiration_date < time:
                ...
                event = {'time':time, 'action':TICKET_EVENT.CHANGED, 'reason':'False positive expired',
                         'reference':None, 'delta':[{'from':True, 'to':False, 'key':'false_positive'}]}
    ...
    def __generate_ticket_details(self, vuln, ticket, check_for_changes=True):
        ...
        if check_for_changes:
            ...
            if delta:
                event = {'time':vuln['time'],'action':TICKET_EVENT.CHANGED,'reason':'details changed', 'reference':vuln['_id'], 'delta':delta}
                ticket['events'].append(event)
    ...
    def open_ticket(self, vuln, reason):
        ...
        prev_open_ticket = self.__db.TicketDoc.find_one({'ip_int':long(vuln['ip']),
                                                         'port':vuln['port'],
        ...
        if prev_open_ticket:
            ...
            self.__check_false_positive_expiration(prev_open_ticket, vuln['time'].replace(tzinfo=tz.tzutc()))   # explicitly set to UTC (see CYHY-286)
            # add an entry to the existing open ticket
            event = {'time':vuln['time'],'action':TICKET_EVENT.VERIFIED,'reason':reason, 'reference':vuln['_id']}
        ...
        reopen_ticket = self.__db.TicketDoc.find_one({'ip_int':long(vuln['ip']),
                                                      'port':vuln['port'],
                                                      'protocol':vuln['protocol'],
                                                      'source':vuln['source'], 
                                                      'source_id':vuln['plugin_id'],
        ...
        if reopen_ticket:
            ...
            event = {'time':vuln['time'],'action':TICKET_EVENT.REOPENED,'reason':reason, 'reference':vuln['_id']}
        ...
        event = {'time':vuln['time'],'action':TICKET_EVENT.OPENED,'reason':reason, 'reference':vuln['_id']}
        new_ticket['events'].append(event)
        
        if new_ticket['owner'] == UNKNOWN_OWNER: # close tickets with no owner immediately
            event = {'time':vuln['time'],'action':TICKET_EVENT.CLOSED,'reason':'No associated owner', 'reference':None}
            new_ticket['events'].append(event)
    ...
    def close_tickets(self):
        if self.__closing_time == None:
            self.__closing_time = util.utcnow() # You don't have to go home but you can't stay here
        ...
        for raw_ticket in tickets:
            ...
            self.__check_false_positive_expiration(ticket, self.__closing_time.replace(tzinfo=tz.tzutc()))   # explicitly set to UTC (see CYHY-286)
            if ticket['false_positive'] == True:
                event = {'time':self.__closing_time,'action':TICKET_EVENT.UNVERIFIED,'reason':reason, 'reference':None}
            else:
                ticket['open'] = False
                ticket['time_closed'] = self.__closing_time
                event = {'time':self.__closing_time,'action':TICKET_EVENT.CLOSED,'reason':reason, 'reference':None}
            ticket['events'].append(event)
            ticket.save()
            
    def ready_to_clear_vuln_latest_flags(self):
        return len(self.__ips) > 0 and len(self.__ports) > 0 and len(self.__source_ids) > 0
...
class IPPortTicketManager(object):
    ...
    def __check_false_positive_expiration(self, ticket, closing_time):
        ...
        if ticket['false_positive'] == True:
            ...
            if fp_expiration_date < closing_time:
                ticket['false_positive'] = False
                event = {'time':closing_time, 'action':TICKET_EVENT.CHANGED, 'reason':'False positive expired',
                         'reference':None, 'delta':[{'from':True, 'to':False, 'key':'false_positive'}]}
                ticket['events'].append(event)
    ...
    def __handle_ticket_port_closed(self, ticket, closing_time):
        ...
        self.__check_false_positive_expiration(ticket, closing_time.replace(tzinfo=tz.tzutc()))   # explicitly set to UTC (see CYHY-286)
        if ticket['false_positive'] == True:
            event = {'time':closing_time,'action':TICKET_EVENT.UNVERIFIED,'reason':reason, 'reference':None}
        else:
            ticket['open'] = False
            ticket['time_closed'] = closing_time
            event = {'time':closing_time,'action':TICKET_EVENT.CLOSED,'reason':reason, 'reference':None}
        ticket['events'].append(event)
        ticket.save()
    ...
    def close_tickets(self, closing_time=None):
        ...
        if all_ports_scanned:
            ...
            tickets_to_close = self.__db.TicketDoc.find({'ip_int':{'$in':ips_with_no_open_ports_ints},
                                                         'open':True})
    ...
    def clear_vuln_latest_flags(self):
        ...
        vuln_docs = self.__db.VulnScanDoc.find({'ip_int':{'$in':ip_ints}, 'latest':True})
