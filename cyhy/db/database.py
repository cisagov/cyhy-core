...
def db_from_config(config_section=None, config_filename=None, yaml=False):
    if yaml:
        if config_section is None: config_section = YamlConfig.DEFAULT
        config = YamlConfig(config_filename).get_service(YamlConfig.MONGO, config_section)
...
def run_pipeline((pipeline, collection), db):
    ...
    try:
        ...
        if e.details['code'] == 16389:
            e.args += ('To avoid this error consider calling run_pipeline_cursor instead.',)
        raise e
...
def ensure_indices(db, foreground=False):
    ...
    for class_name, clazz in db.connection._registered_documents.items():
        ...
        for name, spec, unique, sparse in indices:
            print >> sys.stderr, '\t%s:\tunique=%s\tsparse=%s\t%s ...' % (name, unique, sparse, spec),
            db[class_name].collection.ensure_index(spec, name=name, background=background, unique=unique, sparse=sparse)
...
class HireDoc(RootDoc):
    ...
    def update_stage_time(self):
        ...
        difference_in_weeks = abs(insert_weeks- (today_weeks + (52 * (today_years - insert_years))))
...
class TicketDoc(RootDoc):
    ...
    required_fields = ['source', 'owner', 'ip', 'ip_int', 'port', 'protocol', 'last_change',
                       'time_opened', 'open', 'false_positive', 'source_id', 'details', 'events']
    default_values = {
        'last_change': util.utcnow,
    ...
    def last_detection_date(self):
        for event in reversed(self['events']):
            if event['action'] in [TICKET_EVENT.OPENED, TICKET_EVENT.VERIFIED, TICKET_EVENT.REOPENED]:
                return event['time']
    ...
    def tag_open(self, owners, snapshot_oid):
        self.collection.update({'open':True, 'owner':{'$in':owners}},
                    {'$push':{'snapshots':snapshot_oid}},
                    multi=True,
                    safe=True)

    def tag_matching(self, existing_snapshot_oids, new_snapshot_oid):
        self.collection.update({'snapshots':{'$in':existing_snapshot_oids}},
                    {'$push':{'snapshots':new_snapshot_oid}},
                    multi=True,
                    safe=True)

    def remove_tag(self, snapshot_oid):
        self.collection.update({'snapshots':snapshot_oid},
                    {'$pull':{'snapshots':snapshot_oid}},
                    multi=True,
                    safe=True)

    def get_indices(self):
        return (('ip_port_protocol_source_open_false_positive', [('ip_int',1), ('port',1), ('protocol',1), ('source',1), ('source_id',1), ('open',1), ('false_positive',1)], False, False),
    ...
    def add_event(self, action, reason, reference=None, time=None, delta=None, expires=None):
        if action not in TICKET_EVENT:
            raise Exception('Invalid action "' + action + '" cannot be added to ticket events.')

        if not time:
            time = util.utcnow()
        event = {'time':time, 'action':action, 'reason':reason, 'reference':reference}
        if delta:
            event['delta'] = delta
        if expires:
    ...
    def set_false_positive(self, new_state, reason, expire_days):
        if self['false_positive'] == new_state:
            return
        delta = [{'from':self['false_positive'], 'to':new_state, 'key':'false_positive'}]
        self['false_positive'] = new_state
        now = util.utcnow()
        expiration_date = None
        if new_state:   # Only include the expiration date when setting false_positive to True
            expiration_date = now + datetime.timedelta(days=expire_days)
            if not self['open']:    # if ticket is not open, then re-open it; false positive tix should always be open
                self['open'] = True
                self['time_closed'] = None
                self.add_event(TICKET_EVENT.REOPENED, 'setting false positive', time=now)
        self.add_event(TICKET_EVENT.CHANGED, reason, delta=delta, time=now, expires=expiration_date)
    ...
    def latest_vuln(self):
        ...
        if not vuln:
            ...
            raise VulnScanNotFoundException(ticket_id=self['_id'], vuln_scan_id=reference_id, vuln_scan_time=event['time'])
        return vuln
...
class ScanDoc(RootDoc):
    ...
    def reset_latest_flag_by_owner(self, owner): #TODO could use an index
        self.collection.update({'latest':True, 'owner':owner},
                    {'$set':{'latest':False}},
                    multi=True,
                    safe=True)
    ...
    def reset_latest_flag_by_ip(self, ips):
        ...
        if isinstance(ips, Iterable):
            ...
            ip_ints = [int(ips)]
        self.collection.update({'latest':True, 'ip_int':{'$in':ip_ints}},
                      {'$set':{'latest':False}},
                      multi=True,
                      safe=True)

    def tag_latest(self, owners, snapshot_oid):
        self.collection.update({'latest':True, 'owner':{'$in':owners}},
                    {'$push':{'snapshots':snapshot_oid}},
                    multi=True,
                    safe=True)
    ...
    def tag_timespan(self, owner, snapshot_oid, start_time, end_time):
        self.collection.update({'time':{'$gte':start_time, '$lte':end_time}, 'owner':owner},
                    {'$push':{'snapshots':snapshot_oid}},
                    multi=True,
                    safe=True)
    ...
    def get_indices(self): # used by all subclasses
        return (('latest_ip', [('latest',1), ('ip_int',1)], False, False),
...
class PortScanDoc(ScanDoc):
    ...
    def get_indices(self):
        ...
        return super_indices + \
            (('latest_owner_state', [('latest',1), ('owner',1), ('state',1)], False, False),
    ...
    def get_open_ports_for_ip(self, ip):
        ip_int = int(ip)
        rs = self.find(spec = {'ip_int':ip_int, 'state':'open', 'latest':True},
    ...
    def tag_latest_open(self, owners, snapshot_oid):
        self.collection.update({'latest':True, 'owner':{'$in':owners}, 'state':'open'},
                    {'$push':{'snapshots':snapshot_oid}},
                    multi=True,
                    safe=True)
...
class VulnScanDoc(ScanDoc):
    ...
    def get_indices(self):
        ...
        return super_indices + \
            (('owner_latest_severity', [('owner',1), ('latest',1), ('severity',1)], False, False),)
...
class HostDoc(RootDoc):
    ...
    structure = {
        ...
        'priority': int,
        'r': float,
        'latest_scan': {STAGE.NETSCAN1:datetime.datetime, STAGE.NETSCAN2:datetime.datetime, STAGE.PORTSCAN:datetime.datetime, STAGE.VULNSCAN:datetime.datetime, STATUS.DONE:datetime.datetime},
    }
    required_fields = ['_id', 'ip', 'owner', 'last_change', 'state', 'stage', 'status', 'priority', 'r']
    default_values = {
        'last_change':util.utcnow,
        'priority':0,
        'r':random.random,
        ...
        'latest_scan':{STAGE.NETSCAN1:None, STAGE.NETSCAN2:None, STAGE.PORTSCAN:None, STAGE.VULNSCAN:None, STATUS.DONE:None},
        }

    def save(self, *args, **kwargs):
        self['last_change'] = util.utcnow()
        self['_id'] = long(self['_id']) # force long since mongo import/export don't preserve type info
        try: #TODO handle this more gracefully
            self['loc'] = [float(self['loc'][0]), float(self['loc'][1])] # same as previous
        except TypeError, e:
            print e, self['loc']
        super(HostDoc, self).save(*args, **kwargs)

    def get_indices(self):
        return (('claim', [('status',1), ('stage',1), ('owner',1), ('priority',1), ('r',1)], False, False),
                ('ip', [('ip',1)], False, False),
                ('up', [('state.up',1), ('owner',1)], False, False),
                ('next_scan', [('next_scan',1), ('state.up',1), ('status',1)], False, True),
                ('owner', [('owner',1)], False, False),
                ('latest_scan_done', [('owner',1), ('latest_scan.DONE',1)], False, False),
                ('latest_scan_vulnscan', [('owner',1), ('state.up',1), ('latest_scan.VULNSCAN',1)], False, False))
    ...
    def get_some_for_stage(self, stage, count, owner=None, waiting=False):
        ...
        if owner != None:
            rs = self.find(spec={'status':status, 'stage':stage, 'owner':owner},
                              fields={'ip':True},
                              sort=[('priority', 1), ('r', 1)],
                              limit=count)
        else:
            rs = self.find(spec={'status':status, 'stage':stage},
                              fields={'ip':True},
                              sort=[('priority', 1), ('r', 1)],
                              limit=count)
        return rs
    ...
    def increase_ready_hosts(self, owner, stage, count):
        hosts = self.find(spec={'owner':owner, 'stage':stage, 'status':STATUS.WAITING},
                                  sort=[('priority', 1), ('r', 1)],
                                  limit=count)
    ...
    def decrease_ready_hosts(self, owner, stage, count):
        hosts = self.find(spec={'owner':owner, 'stage':stage, 'status':STATUS.READY},
                                  sort=[('priority', 1), ('r', 1)],
                                  limit=count)
        # important to count before using cursor
        changed_count = hosts.count(True)
    ...
    def reset_state_by_owner(self, owner, init_stage, jump_start=False):
        ...
        now = util.utcnow()
        if jump_start:
            self.collection.update(spec={'owner':owner, 'state.up':True},
                        document={'$set': {'stage':STAGE.PORTSCAN, 'status':STATUS.WAITING, 'last_change':now, 'next_scan':None}},
                        multi=True)
            self.collection.update(spec={'owner':owner, 'state.up':False},
                        document={'$set': {'stage':init_stage, 'status':STATUS.WAITING, 'last_change':now, 'next_scan':None}},
                        multi=True)
        else:
            self.collection.update(spec={'owner':owner},
                    document={'$set': {'stage':init_stage, 'status':STATUS.WAITING, 'last_change':now, 'next_scan':None}},
                    multi=True)
    ...
    def get_scheduled_hosts(self, state_up, time=None, limit=2000):
        ...
        if time == None:
            time = util.utcnow()
        cursor = self.find(spec={'next_scan':{"$lte":time}, 'state.up':state_up, 'status':STATUS.DONE}).limit(limit)
        return cursor

    def purge_all_running(self):
        now = util.utcnow()
        self.collection.update(spec={'status':STATUS.RUNNING},
                    document={'$set': {'status':STATUS.WAITING, 'last_change':now}},
                    multi=True)

    def ensure_next_scan_set(self, owner):
        now = util.utcnow()
        self.collection.update(spec={'next_scan':None, 'owner':owner},
                    document={'$set': {'next_scan':now}},
                    multi=True)

    def clear_next_scan_date(self, owner):
        self.collection.update(spec={'next_scan':{'$exists':True}, 'owner':owner},
...
class RequestDoc(RootDoc):
    ...
    required_fields = [
        ...
        'period_start', 'windows', 'init_stage', 'stakeholder'
        ]
    default_values = {
    ...
    def add_children(self, db, child_ids):
        ...
        for child in child_ids:
            if child == self['_id']:
                raise ValueError('Cannot add own id (' + child + ') to list of children')
            if self.get('children') and child in self['children']:
                raise ValueError('Child (' + child + ') cannot be added; it is already in list of children of ' + self['_id'])
            if not db.requests.find_one({'_id':child}):
                raise ValueError('Child (' + child + ') was not found in the database and cannot be added')
    ...
    def remove_children(self, child_ids):
        ...
        for child in child_ids:
            if child not in self['children']:
                raise ValueError('Child (' + child + ') cannot be removed; it is not in list of children of ' + self['_id'])
        self['children'] = list(set(self['children']) - set(child_ids))
        return True

    def get_all_descendants(self, owner, stakeholders_only=False, include_retired=False):
        # Build dict of every org and its children (if any)
        org_info = dict()
        for org in self.find():
            org_info[org['_id']] = {'children':org.get('children', []),
        ...
        def get_descendants(org_info, current_org, stakeholders_only, include_retired):
            ...
            for child_org in org_info.get(current_org, {}).get('children', []):
                if include_retired or not org_info[child_org].get('retired'):
                    ...
                    descendants = descendants.union(get_descendants(org_info,
                                                                    child_org,
                                                                    stakeholders_only,
                                                                    include_retired))
            return descendants
    ...
    def get_owner_to_type_dict(self, stakeholders_only=False, include_retired=False):
        ...
        for agency_type in AGENCY_TYPE:
            all_agency_type_descendants = self.get_all_descendants(agency_type, include_retired=include_retired)
    ...
    def get_owner_types(self, as_lists=False, stakeholders_only=False, include_retired=False):
    ...
    def save(self, *args, **kwargs):
        if self['agency'].get('location'):
            self['agency']['location']['gnis_id'] = long(self['agency']['location']['gnis_id'])
...
class TallyDoc(RootDoc):
    ...
    def active_host_count(self, stage):
        counts = self['counts']
        return (counts[stage][STATUS.WAITING], counts[stage][STATUS.READY], counts[stage][STATUS.RUNNING])
...
class SnapshotDoc(RootDoc):
    ...
    def get_indices(self):
        return (('uniques', [('owner',1), ('start_time',1), ('end_time',1)], True, False),
                ('latest_owner', [('latest',1), ('owner',1)], False, False))
    ...
    def will_conflict(self):
        ...
        results = self.collection.find_one({'_id':{'$ne':self['_id']},
                                 'owner':self['owner'],
...
class ReportDoc(RootDoc):
    ...
    default_values = {
        'generated_time': util.utcnow
    }

    def get_indices(self):
        return (('owner', [('owner',1)], False, False),
