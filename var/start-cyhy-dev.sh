...
# (includes more scripts than start-cyhy-ops.sh)
# Usage:
#    source start-cyhy-dev.sh
...
# directory or a sub-directory of it.  Choose your CYHY_HOME_DIR wisely.
...
docker run --rm --name cyhy-core --detach --volume $CYHY_ETC_DIR:/etc/cyhy --volume $CYHY_HOME_DIR:/home/cyhy $CYHY_CORE_IMAGE

# Create aliases for all of the cyhy commands
eval "$(docker exec cyhy-core getenv)"

alias cyhy-bash="docker exec -it cyhy-core /bin/bash"
